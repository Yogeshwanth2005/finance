...
AMFI_HISTORY_URL = "https://portal.amfiindia.com/DownloadNAVHistoryReport_Po.aspx"  # dated NAV report: ?frmdt=DD-Mon-YYYY&todt=DD-Mon-YYYY, every scheme
AMFI_FETCH_TIMEOUT_SECONDS = 120  # a dated report of a week or two is 6 to 7 MB
...
FUND_END_RANGE_DAYS = 7  # the newest NAVs come from the last this-many days of AMFI's dated reports
FUND_WINDOW_LOOKBACK_DAYS = 4  # a window's base NAV is looked for this many days before the window's start day
FUND_CACHE_TTL_HOURS = 24
FUND_CATALOG_MIN_KEEP_RATIO = 0.5  # a catalog with fewer schemes than this share of the stored rows is a bad download, not a mass delisting
FUND_REFRESH_RETRY_MINUTES = 5  # after a failed refresh or first-NAV pass, wait this long before trying again
FUND_FIRST_NAV_START = "2013-01"  # Direct plans start in January 2013: the oldest month-start snapshot the first-NAV pass reads
FUND_CHECKPOINT_MIN_ROWS = 1000  # a weekday's report counts as a snapshot only with this many schemes (a Sunday's or a holiday's holds about 600)
FUND_CHECKPOINT_MAX_DAY_TRIES = 7  # calendar days from the 1st of a month the pass will look at for a snapshot
FUND_FIRST_NAV_MAX_FAILURES = 3  # consecutive failed snapshots that stop a first-NAV pass
FUND_REPO_BATCH_SIZE = 500  # documents per bulk write
