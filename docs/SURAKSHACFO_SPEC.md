...
3. `/insurance` shows plan filters, comparison guidance and streams contextual answers through `POST /api/chat/stream`. Plan cards come from `GET /api/plans` (only cards an admin published from indexed documents; an empty state shows until there are any). Clicking a card opens its details and an "Ask the advisor about this plan" shortcut.
