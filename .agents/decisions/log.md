...
## Decisions
- **2026-09-24** | Moved **everything investment-related off the dashboard onto a new `/investments` page**: the Surplus direction card (now `components/SurplusDirectionCard.tsx`), `FundExplorer`, and the "Investable next" metric. The dashboard keeps protection, cashflow, the KPI grid and the insurance status card, plus a link card to `/investments` where the investment card used to be; `AppShell` gets an Investments tab between Dashboard and Insurance. Frontend only, no API change | **why**: user request ("keep about investments in a separate page"); asked which parts, answer was "everything investment-related" | **assumptions, not from a source**: (1) the cashflow card's "Investable surplus" row stays on the dashboard, since "where every rupee goes" would not add up without it (the user was not asked about this row); (2) the page's `block` value (`"emergency"` / `"insurance"` / `null`) is computed once in `Investments.tsx` and passed to both the card and `FundExplorer`, instead of each recomputing the same test; (3) the translation key `investableNext` is now unused in `i18n.ts` and was left in place; (4) `FundExplorer`'s blocker note still says "close the gap above", which still reads right because the Surplus direction card sits directly above it on the new page | **rejected**: moving only the Fund Explorer (the user chose the wider scope); leaving the "Protect, then invest" section as one lone insurance card (a link card to `/investments` keeps the two-column layout and the path forward) | **found in the browser, fixed**: the nav tab and page heading first read "Current investments (₹)": `AppShell` looked labels up with `t(label.toLowerCase())`, and the existing `investments` key is the profile wizard's input label, not "Investments". Fixed with a new `investmentsPage` key (hi निवेश, te పెట్టుబడులు, ta முதலீடுகள்) and an explicit `labelKey` on every nav item, so no nav label is guessed from English text any more | **verification**: typecheck clean, `oxlint` clean on the touched files (the repo's 4 old warnings remain), 13 Vitest tests pass, `vite build` ok; the real app was driven in a browser (local MongoDB, throwaway user, deleted afterwards) through all three states of the card: emergency-fund blocker, insurance blocker (surplus ₹0), and the SIP plan (₹1.5L a month, rows matching the API's 39 / 16.2 / 9.8 / 25 / 10 split, goal and crash lines), with the "protection first" note on Fund Explorer only in the two blocked states; the Dashboard shows no fund or allocation content and its link card opens `/investments`; the four fund lists loaded from stored rows; the nav and heading were checked in en, hi, te and ta; no console errors and every `/api` call returned 200 | **not yet done / not verified**: no Vitest test covers the new page or the moved card (the repo has none for pages); the hi / te / ta words for "Investments" have had no native review, and the page subheading is English only; only a desktop-width viewport was looked at, not a phone.
- **2026-09-24** | Built **all funds by fund house, stored in MongoDB** (plan `docs/superpowers/plans/2026-09-24-all-funds-stored-cache.md`, spec `docs/superpowers/specs/2026-09-24-all-funds-stored-cache-design.md`, both local: `docs/` is git-ignored). Searching a fund house now lists every live Direct Growth fund it has, whatever its category: the four equity segments first, then one card per AMFI category A to Z, each showing the selected window's return or "—". `lib/funds.py` keeps AMFI's `NAVAll.txt` as the catalog (now every live Direct + Growth scheme, with AMFI's own category label) and computes 1Y / 3Y / 5Y from AMFI's **dated NAV reports** (`DownloadNAVHistoryReport_Po.aspx?frmdt=&todt=`, one call for all schemes; four downloads per refresh: the newest 7 days and a range around each window's start); new `lib/fund_repo.py` (`fund_rows`, one ~400-byte document per scheme, ISO-string dates, the only code that touches the collection) and `lib/fund_store.py` (`FundStore`: loads the stored rows at boot so a woken server answers at once, atomic refresh at most once per 24 h, and a background first-NAV pass that reads month-start snapshots from January 2013 to give the Max window a start); `/api/funds/search` returns ordered `sections`, both endpoints return `max_pending`. **This reverses the same day's "storing fund data in Mongo" rejection and removes mfapi.in and the per-fund crawl** (the fund explorer entry below is kept as history) | **why**: user request: searching a fund house should show *all* its funds, stored in the database and updated day to day. The crawl took ~135 s on every Render wake (the free tier wipes memory when it sleeps), left 1 to 6 funds out, and a fund-house search showed 4 HDFC funds though HDFC has 89 Direct Growth funds | **measured on the real data, 2026-09-24**: 1,785 live Direct Growth schemes from 53 fund houses (HDFC 89, Kotak Mahindra 98); a weekday's dated report holds 8,601 schemes and a Sunday's 601, so `FUND_CHECKPOINT_MIN_ROWS` is **1000, not the spec's 100**, and Saturdays and Sundays are never downloaded (100 would accept a Sunday and find every equity fund launched that month a month late); a full refresh took 21.2 s and 34.8 MB into the local MongoDB (719 KB in `fund_rows`) and a simulated restart loaded all 1,785 rows in 16 ms with nothing downloaded; the first-NAV pass took 2 min 50 s over 181 one-day downloads (about 1 MB each, not byte-counted), leaving 1,758 first NAVs from snapshots and 27 `first_seen`; the smallest snapshot accepted was about 2,450 schemes (quarter-start days) and the only ones skipped for being under 1000 were 1 May 2013 (697) and 1 May 2014 (841), both Maharashtra Day exchange holidays, so the threshold stands; 1,599 funds have a 1Y return, 1,206 a 3Y and 870 a 5Y (the plan's "about 1,780 with a 3Y" was an unmeasured guess: the counts are an age curve, and the equity funds without a 3Y are recent launches, checked against mfapi for two of them); 67 fund-windows of 1Y / 3Y / 5Y matched mfapi.in to 0.000 percentage points on the same end date; for 14 funds the stored first NAV gives a Max within 0.7 points of mfapi's oldest NAV (a 2013-era fund's first snapshot is 1 Feb where mfapi's first NAV is 2 Jan, because 1 Jan is a partial day) | **assumptions, not from a source**: (1) window and range sizes: `end` = the last 7 days, and for N years `T_N = newest - round(N × 365.25)` days with a range of T_N − 4 to T_N + 7, one date for every fund; a base NAV is the last on or before T_N, else the first within 7 days after; (2) a move over 35% between two consecutive NAVs voids that range (the end range voids every window); (3) Max is annualised from a fund's first NAV seen on a month-start snapshot, so it is approximate and up to about a month late; a fund under a year old shows its absolute return, labelled; (4) on the very first fill no scheme gets a first NAV (stamping "first seen today" would make every Max wrong), a scheme new to a store that already had rows gets the earliest NAV of the end range; (5) ETFs stay out (AMFI gives them no Direct/Regular label); (6) search shows every fund including those with no value for the window, after the ranked ones; sections use AMFI's own labels, so near-duplicates such as "FoF Domestic" and "Fund of Funds Scheme (Domestic)" are separate cards; (7) a catalog with fewer than half as many schemes as are stored is a bad download, not a mass delisting (`FUND_CATALOG_MIN_KEEP_RATIO`), added in the final review after a probe showed a single future-dated NAV in AMFI's file makes every other scheme look dead, collapsing the catalog from 12 schemes to 1, which would have replaced the stored rows and deleted the first NAVs while reporting `ready` | **rejected**: mfapi.in for returns or history (slow, trails AMFI by about 5 days, and its per-fund crawl is what this replaces); Groww's API (a trading API with no mutual-fund scheme data); storing NAV history (only computed returns and each fund's first NAV are stored); a scheduled midnight trigger (a follow-up, see the backlog) | **verification**: 355 backend unit tests pass (276 before), frontend typecheck clean, `oxlint` shows only its 4 existing warnings, 13 Vitest tests (7 new), `vite build` ok; the repository's first-NAV rules (`$setOnInsert`, the `first_nav: null` filter, the empty-keep-set refusal) were run against a real local MongoDB; the real app ran against live AMFI: four top-10 lists at once from stored rows, `hdfc` gave "Results for “hdfc” · HDFC · 89 funds" with one card per section, the 1Y / 3Y / 5Y / Max toggle re-ranked with "—" rows last, "Back to top 10" cleared the box, `zzzz` gave the no-match line, the Max note changed from "still being calculated" to the approximate text when the pass ended, no console errors, and a backend restart made no request to AMFI | **not yet done / not verified**: AMFI reachability from Render (fetched from the dev machine only), Atlas M0 behaviour and Render's wake time, a multi-day refresh across real sleeps, the live-server suite (`tests/test_tscheck_*`); nothing was pushed or deployed. Deferred minors: if a refresh lands while the first-NAV pass is saving, the refresh's `$set` can overwrite `returns.max` in Mongo with `None` while `first_nav` stays (memory is right, the next refresh repairs it); `parse_navall` still takes "newest" from the largest date in the file, so a future-dated row makes every refresh fail with the rows kept and the status `stale`, until AMFI corrects it.
- **2026-09-24** | *(The fund data source, the in-process cache and the "not stored in Mongo" rejection below were superseded later the same day by the all-funds entry above; the goal check, the equity split and the UI parts stand.)* Built the **fund explorer and inflation-goal check** (plan `docs/superpowers/plans/2026-09-24-fund-explorer-and-goal-check.md`, spec beside it; `docs/` is git-ignored, so both stay local). Engine: `lib/allocation.py` gains `compute_equity_split` (large / mid / small share of the equity slice by risk tolerance) and `compute_goal_check` (expected return of the mix against inflation plus a risk-scaled margin, the equity share that would reach the goal, and a March-2020-style crash line); `analysis.allocation` is unchanged and `analysis` gains `equity_split` and `goal_check`. Funds: `lib/funds.py` parses AMFI's `NAVAll.txt` as the catalog (Direct + Growth; large, mid, small cap and plain Nifty 50 index funds), pulls each fund's NAV history from mfapi.in for 1Y / 3Y / 5Y / Max returns, and keeps it all in an in-process cache (24 h, warmed from `lifespan`, single-flight refresh, a failed refresh keeps the last data as `stale`, no retry sooner than 5 min); `GET /api/funds/top` and `/api/funds/search` need a signed-in user. UI: the dashboard investment card shows five rows (Large / Mid / Small cap, Debt, Gold) plus a goal line and a crash line, and `FundExplorer` shows the top 10 per segment with a 1Y / 3Y / 5Y / Max toggle and a fund-house search. **This supersedes the 2026-09-23 curated-15-fund plan** (entry below): AMFI's file carries the SEBI category and the fund house for every scheme, which removes the "38,000 detail calls" obstacle that killed "show every fund" | **why**: user request: show a family how to split a monthly SIP so the money beats inflation with a margin, and let them browse real funds for each slice | **findings from the real data**: (1) on 2026-09-24 the catalog is 129 funds (34 large, 33 mid, 36 small, 26 Nifty 50) from 45 fund houses, not the spec's estimate of about 31 / 31 / 32 and 267 index funds; (2) AMFI labels the growth option several ways (`Growth`, `GROWTH`, `Direct Growth`, and ICICI Prudential's `Cumulative`), so the parser is case-insensitive and accepts `cumulative`, and without that a whole fund house vanishes silently; (3) the Nifty 50 rule is `\bnifty\s*50\b` or SBI-style "Nifty Index", and the variant words (next, equal, value, quality, alpha, momentum, low vol, dividend) are looked for only after the word "Nifty", because a fund house can itself be called "Alpha"; (4) mfapi.in trails AMFI by about 5 days (newest NAV 18 Sep while AMFI's was 23 Sep); (5) a cold warm-up took about 135 s here (AMFI file, then ~129 history calls at concurrency 6), longer than the ~75 s the plan estimated, and 1 to 6 funds were left out on each of three runs because some mfapi calls run close to the 60 s timeout; the UI covers it with a "Refreshing fund data…" state polled every 5 s | **assumptions, not from a source**: (1) cap split: 40% of the equity slice is "growth" at a 1.0 risk multiplier, scaled by the multiplier, and mid gets 5/8 of it (large / mid / small of the equity slice: 68 / 20 / 12 conservative, 60 / 25 / 15 moderate, 52 / 30 / 18 aggressive); (2) inflation 6%, a margin of 3 / 4 / 5 points by risk tolerance, and expected returns large 11, mid 13, small 14, debt 7, gold 8: deliberately not measured past returns, because gold's recent run would steer every profile into gold; (3) stress falls large 37, mid 36, small 38, debt and gold 0: the Jan to Apr 2020 median worst fall of catalog funds, re-measured on 2026-09-24 as large 36.5 (n=24), mid 35.5 (n=20), small 37.7 (n=19), Nifty 50 37.9 (n=12), so the constants stand; debt and gold were assumed flat, not measured, and Direct plans only date from 2013, so the data holds one crash and no 2008; (4) a single move over 35% between two NAVs drops a fund as a data glitch, a scheme is live when its NAV is within 7 days of the newest in the file, a fund launched within 7 days after a window's start still gets that window; (5) lists are ranked by trailing return with a past-performance note, consistent with the 2026-09-21 entry that superseded the 2026-09-18 objection to ranking, which the 2026-09-23 entry below asked to settle before building | **rejected**: Regular plans and IDCW options (Direct + Growth only, so one fund is not listed four times); storing fund data in Mongo (Render's free tier wipes memory on every wake, so the accepted cost is a cold start); AUM, expense ratio, ratings and minimum SIP (neither source carries them); retiring `select_fund_examples` (left untouched, still unwired); pre-built "combinations" (the user deferred them on 2026-09-23) | **verification**: 276 backend unit tests pass (103 before, 173 new); frontend typecheck clean, `oxlint` shows only its 4 existing warnings, 6 new Vitest tests, `vite build` ok; the real AMFI file was parsed (counts above) and the store warmed against live AMFI and mfapi.in; the dashboard was driven in a browser against the running backend: five rows adding to 100, the goal and crash lines for age 35 and age 50 and horizons of 10 and 3 years, the emergency-fund blocker (goal lines gone, explorer note shown), the warming message then an automatic fill-in, the four-way toggle, a search that matches (hdfc) and one that does not (zzzz), and "Back to top 10"; no console errors from the app | **not yet done**: AMFI and mfapi.in reachability from Render is untested (fetched from the dev machine only) and Render's cold-start time is unmeasured; the live-server suite (`tests/test_tscheck_*`) was not run; the assumed returns and stress falls have no source or owner; SIP management (CAS import) is still only an idea.
