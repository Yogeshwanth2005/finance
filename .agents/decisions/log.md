...
## Decisions
- **2026-09-23** | Swapped the LLM seam from Gemini (`google-genai`) to OpenRouter's OpenAI-compatible API, called with plain `httpx` (already a dependency): `lib/llm.py` keeps `llm_configured`, `generate_json` and `stream_answer`; env vars are now `OPENROUTER_API_KEY` / `OPENROUTER_MODEL` (default `openrouter/free`); `google-genai` is out of `requirements.txt`; the chat header label in `i18n.ts` says "OpenRouter"; the adapter tests were rewritten (11, real `httpx` client over `MockTransport`) and moved from `tests/` to `tests/unit/` | **why**: user request: they wanted OpenRouter for its free models, and the previous Gemini calls all failed with `API_KEY_INVALID` | **assumptions, not from a source**: (1) `openrouter/free` as the default, because it routes across whichever free models are live and supports JSON mode; (2) `generate_json` sends `response_format: json_object` and also strips a ```` ```json ```` fence from the reply, since not every free model honours JSON mode; (3) an `error` object in a 200 body or an SSE event raises, because OpenRouter reports provider failures that way | **rejected**: the `openai` SDK (a new dependency for two calls); falling back to `GEMINI_API_KEY` for the new variable (that is how a wrong-provider key got treated as configured); logging changes in `chat.py` (out of scope, noted as debt) | **verification**: 101 unit tests pass (90 before, plus 11 adapter); `import server` is clean. **Not verified live**: the key in `backend/.env` was a Groq `gsk_…` key, and OpenRouter answered 401 "Missing Authentication header" to it through both the adapter and plain curl, while a fake `sk-or-v1-` key got "User not found", so the adapter is not at fault. I moved that value to `GROQ_API_KEY` and left `OPENROUTER_API_KEY=""` | **not yet done**: put a real `sk-or-v1-…` key in `.env` and on Render, run `generate_json` and `stream_answer` live, and confirm a live chat `done` event has no `fallback`; the live-server suite was not run this session.
