...
## Decisions
- **2026-09-21** | Plan cards are generated from uploaded documents with an admin review gate: `_save_document` calls `lib/plan_extract.extract_plan` (Gemini JSON via `llm.generate_json`), stores a draft `plan` + `plan_status` (`none`/`draft`/`published`) on the `rag_documents` record; admins edit/publish via `PUT /admin/documents/{id}/plan`, `PATCH .../plan/status`, `POST .../plan/extract`; customers read published cards of active documents from `GET /plans`; the Insurance page shows them (click opens `PlanDetailDialog`) and falls back to the hardcoded illustrative plans until any exist | **why**: user request: cards like "Click 2 Protect Super" should be automatic from any uploaded plan document, with details on click, and with admin review because extracted premiums/ratios are financial facts an LLM can misread | **assumptions, not from the source app**: (1) only `term`/`health` documents make a card (other types get none); (2) csr and premium are dropped to null rather than guessed when not clearly stated, other facts show "Not stated in the document"; (3) editing a published card keeps it published, re-extracting resets it to draft; (4) "Ask about this plan" sends the *document title* because named-plan matching is by title words | **rejected**: auto-publish (wrong numbers would go live unreviewed); replacing `/profile`'s hardcoded `plans` (kept as the fallback so profile tests and the empty state still work) | **not yet done**: verified with fake-DB route checks, unit tests and a browser run against a mock API only, not against a live key or the live-server suite (`test_tscheck_plan_cards.py` is written but unrun); real-brochure extraction quality is untested; the two hardcoded plan lists (`routers/profile.py`, `sampleData.ts`) are still duplicated.
