...
- LLM: OpenRouter (OpenAI-compatible API over plain `httpx`, default model `openrouter/free`) through `backend/lib/llm.py`, optional. RAG uses local 128-dim hashed bag-of-words vectors stored in Mongo, cosine in Python.
...
4. **`lib/llm.py` is the only module that knows about OpenRouter.** With no `OPENROUTER_API_KEY` chat falls back to a deterministic, localized answer; that path must keep working.
...
- `backend/lib/llm.py` — `llm_configured()`, `generate_json()`, `stream_answer()` (OpenRouter seam)
